...
function scaleUtility(name: string, properties: string | string[]) {
  ...
  const utility = `@utility yhn-${name}-* {
	${propertiesArray
    .map(
      (property) =>
  ...
  const autoCompleteUtility = `@utility yhn-${name}-px {
	${propertiesArray
...
function columnScaleUtility(name: string, properties: string | string[]) {
  const propertiesArray = Array.isArray(properties) ? properties : [properties]
  const utility = `@utility yhn-${name}-col-* {
	${propertiesArray
    .map(
      (property) =>
  ...
  const autoCompleteUtility = `@utility yhn-${name}-col-value {
	${propertiesArray
    .map(
      (property) =>
