# AGENTS.md - YHN Engineering Standards
...
## House Style (YHN conventions)

These are the subjective choices baked into this starter. A team forking YHN can change any of these without breaking the build.
...
- `yhn-*` utility classes for responsive scaling (see `lib/styles/README.md`)
...
bun run YHN        # Plugin CLI: `list`, `add <plugin>` (restore an integration from the YHN repo)
